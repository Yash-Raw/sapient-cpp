...
- **Safetensors JSON accept/reject matches serde_json 1.0.150:** a 3-element array is accepted as `StMeta` (serde's sequence form); a UTF-8-BOM-prefixed header is rejected; nesting deeper than 127 levels (including the root object) is rejected, matching serde's recursion limit; a raw NUL byte anywhere in the header text is rejected (nlohmann's lexer would otherwise treat `\0` as end-of-input and silently accept a truncated or `\0`-padded header). Only the message tail after the SAPIENT-authored prefix differs (exempt, per above).
- **A path containing an embedded NUL byte is rejected before any syscall, on every GGUF/safetensors loader entry point,** exactly as Rust's `File::open`/`fs::read` reject it (a fixed `InvalidInput`-class error, not a raw OS errno — no "(os error N)" suffix). `path.c_str()` has no such check and would otherwise silently truncate to, and open, a different, shorter path than Rust ever sees.
...
| 2026-09-23 | sp1a plan B final-review fix wave — 2 sapient-io Rust tests ported by name + the synthetic GGUF/safetensors/mmap/rust_std suites, now 76 tests total (72 C++-only synthetic-file + 2 `RealFile.*` + 2 ported), incl. 6 new: safetensors NUL-in-header rejection, NUL-in-path rejection on every GGUF/safetensors loader entry point, and an mmap-route non-kept wrapped-range death test (`GgufTensorsWrappedLength`/`GgufTensorsDeferredMmapPanic`, renamed from `GgufTensorsFix1`/`GgufTensorsFix2`) | macOS arm64 (Apple M5) | 52bc163 | 9e1fbbc | pass — `cargo test -p sapient-io`: 2 passed (unchanged, Rust frozen); C++ suite `sapient_io_tests`: 76 registered / 74 run, 100% (2 `RealFile.*` skipped without env vars set); workspace-wide `ctest --preset dev`: 333 registered, 100% of the 332 counted pass (1 `Spinpool.DISABLED_pool_speedup_probe` not run) |
| 2026-09-23 | `RealFile.gguf_heap_and_mmap_agree` on SmolLM2-135M-Instruct Q4_K_M (re-verified after the fix wave) | macOS arm64 | — | 9e1fbbc | pass — `[real-file] …/SmolLM2-135M-Instruct-Q4_K_M.gguf: 272 tensors, 45 zero-copy, 166 requantised Q5_0->Q8_0, 33 KVs` |
| 2026-09-23 | `RealFile.safetensors_loads` on kokoro-82m `model.safetensors` (re-verified after the fix wave) | macOS arm64 | — | 9e1fbbc | pass — `[real-file] …/models--sai1974dev--kokoro-82m-safetensors/…/model.safetensors: 548 tensors` |
