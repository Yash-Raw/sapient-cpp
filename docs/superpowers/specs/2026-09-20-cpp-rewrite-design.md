...
cpp/                                   ← the C++ SAPIENT (this is what ships)
  ...
  tests/parity/           see D1
crates/                                ← Rust oracle, untouched except the dump example
...
| 1a | **Core + IO + CPU kernels** | sapient-core (all), sapient-io (GGUF heap+mmap+metadata, safetensors; ONNX reader stubbed to bytes until #8), backends-cpu (all kernels, **per-ISA dispatch mirrored exactly**: aarch64 NEON/SDOT/SMMLA + Q8_K + R4 repack; x86_64 = scalar K-quants + the two existing AVX2 dots; spinpool, thermal, parking pool, PoolAllocator) | all 72+22+2 unit tests ported; golden-dump bit-identity on both arm64 and x86_64 hosts; `q6_k_scale_indexing_matches_ggml`, `q5_k_dequant_high_bits_per_element`, `leading_masked_positions_do_not_nan` etc. present by name | ~8k |
