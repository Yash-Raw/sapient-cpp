...
Run: `cd cpp && cmake --build --preset dev && ctest --preset dev`
Expected: **253 tests, 100%** (248 after Task 1, plus 5 `Spinpool.*`; the `DISABLED_` probe is not counted). **This is the commit where the default route flips on macOS** — every quant, matmul, attention and golden test now dispatches through the spin pool. If any of the previously-green 242 now fails, the pool is wrong; do not adjust a kernel.
...
Run: `cd cpp && SAPIENT_SPINPOOL=1 SAPIENT_THERMAL=off ctest --preset dev 2>&1 | tail -5` then `SAPIENT_SPINPOOL=0 ctest --preset dev 2>&1 | tail -5`
Expected: 100% both ways. (Task 3 makes this a permanent ctest gate rather than a manual step; running it here catches a broken pool before the wiring task builds on it.)
