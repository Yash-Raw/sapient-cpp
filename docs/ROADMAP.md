...
| 1a | Core + IO + CPU kernels | 96 unit tests ported; golden dumps bit-identical on arm64 and x86_64 | in progress — plans A (core), C (dense kernels), D (quant kernels) and E (spinpool + thermal governor, `for_each_out_chunk` gated bit-identical pool-on/pool-off) implemented on feat/cpp-sp1a; plan B (io) remaining |
