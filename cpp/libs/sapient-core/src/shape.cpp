...
Result<Shape> Shape::broadcast_with(const Shape& other) const {
    ...
    for (size_t i = 0; i < len; ++i) {
        ...
        const size_t bi =
            i < len - other.dims.size() ? 1 : other.dims[i - (len - other.dims.size())];
        if (ai == bi)
            out[i] = ai;
        else if (ai == 1)
            out[i] = bi;
            ...
            return tl::unexpected(Error::broadcast(dims, other.dims));
    }
    ...
    for (size_t i = 0; i < dims.size(); ++i)
        if (dims[i] == 0)
            return tl::unexpected(
                Error::invalid_graph("Shape has zero dimension at axis " + std::to_string(i)));
    return {};
}

Result<size_t> Shape::flat_index(std::span<const size_t> idx) const {
    if (idx.size() != dims.size())
        return tl::unexpected(Error::rank_mismatch(dims.size(), idx.size()));
    const auto st = strides();
    size_t off = 0;
    for (size_t i = 0; i < idx.size(); ++i) {
        if (idx[i] >= dims[i])
            return tl::unexpected(Error::internal("Index " + std::to_string(idx[i]) +
                                                  " out of bounds for dim " + std::to_string(i) +
                                                  " (size " + std::to_string(dims[i]) + ")"));
        off += idx[i] * st[i];
    }
    return off;
}
